...
def test_is_positive_passes_for_positive_integers(value: int):
    metadata = IsPositive()
...
def test_is_positive_fails_for_non_positive_integers(value: int):
    metadata = IsPositive()
...
def test_is_negative_passes_for_negative_integers(value: int):
    metadata = IsNegative()
...
def test_is_negative_fails_for_non_negative_integers(value: int):
    metadata = IsNegative()
...
@given(value=st.integers(min_value=0))
def test_is_non_negative_passes_for_non_negative_integers(value: int):
...
@given(value=st.integers(max_value=-1))
def test_is_non_negative_fails_for_negative_integers(value: int):
    metadata = IsNonNegative()
...
@given(value=st.integers(max_value=0))
def test_is_non_positive_passes_for_non_positive_integers(value: int):
...
@given(value=st.integers(min_value=1))
def test_is_non_positive_fails_for_positive_integers(value: int):
    metadata = IsNonPositive()
...
def test_precision_validates_integers_regardless_of_limit(
    value: float, max_decimals: int
):
...
def test_after_date_passes_for_dates_after_threshold(threshold: date, days_after: int):
...
def test_after_date_fails_for_dates_not_after_threshold(
    threshold: date, days_before: int
):
    metadata = AfterDate(after=threshold)
...
def test_before_date_passes_for_dates_before_threshold(
    threshold: date, days_before: int
):
...
def test_before_date_fails_for_dates_not_before_threshold(
    threshold: date, days_after: int
):
    metadata = BeforeDate(before=threshold)
    value = date.fromordinal(threshold.toordinal() + days_after)
...
def test_date_range_validates_dates_within_exclusive_range(after: date, before: date):
    metadata = DateRange(
...
def test_unique_items_passes_for_unique_lists(items: list[int]):
...
def test_unique_items_fails_for_lists_with_duplicates(items: list[int], duplicate: int):
    metadata = UniqueItems()
...
def test_sequence_contains_passes_when_item_present(
    items: list[int], required_item: int
):
...
def test_item_type_passes_for_correct_types(items: list[int]):
...
def test_required_keys_passes_when_all_present(keys: list[str], data: st.DataObject):
...
def test_required_keys_fails_when_missing(required: list[str], data: st.DataObject):
...
def test_forbidden_keys_passes_when_none_present(
    forbidden: list[str], data: st.DataObject
):
...
def test_forbidden_keys_fails_when_present(forbidden: list[str], data: st.DataObject):
    metadata = ForbiddenKeys(keys=tuple(forbidden))
...
def test_min_keys_validates_correctly(min_keys: int, actual_keys: int):
...
def test_max_keys_validates_correctly(max_keys: int, actual_keys: int):
...
def test_numeric_validators_pass_none_values(validator_choice: str):
    validators = {
...
def test_date_validators_pass_none_values(threshold: date):
...
def test_sequence_validators_pass_none_values(items: list[int]):
...
def test_mapping_validators_pass_none_values(keys: list[str]):
...
@given(value=st.integers(max_value=0))
def test_errors_are_non_empty_string_tuples_numeric(value: int):
    metadata = IsPositive()
...
def test_errors_are_non_empty_string_tuples_datetime(threshold: date, days_after: int):
    metadata = BeforeDate(before=threshold)
    value = date.fromordinal(threshold.toordinal() + days_after)
...
def test_errors_are_non_empty_string_tuples_sequence(items: list[int], duplicate: int):
    metadata = UniqueItems()
...
def test_errors_are_non_empty_string_tuples_mapping(required: list[str]):
    metadata = RequiredKeys(keys=tuple(required))
